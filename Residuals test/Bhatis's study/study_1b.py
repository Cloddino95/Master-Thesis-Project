...
for model_name, model in tqdm(models.items()):
    ...
    plt.title(f'(Study 1B) Residuals vs Predicted Values for {model_name} with Parameter={model_param}')
    plt.show()

    #plt.hist(residuals, bins=30)
    #plt.xlabel('Residual')
    #plt.title(f'(Study 1B) Histogram of residuals for {model_name} with Parameter={model_param}')
    #plt.show()
