...
data_300dim_2 = mat_closest_word_vectors_2.copy()
...
Ridge_10W = Ridge().set_params(alpha=4)
Ridge_10W.fit(X_train, y_train)
Ridge_pred_test10W = Ridge_10W.predict(X_test)
Ridge_pred_train10W = Ridge_10W.predict(X_train)

SVR_RBF_10W = SVR(kernel='rbf').set_params(C=100)
SVR_RBF_10W.fit(X_train, y_train)
SVR_RBF_pred_test10W = SVR_RBF_10W.predict(X_test)
SVR_RBF_pred_train10W = SVR_RBF_10W.predict(X_train)


data_SVR = {"names": y_test.index.tolist(), "(SVR-RBF) predicted values": pd.Series(SVR_RBF_pred_test10W.flatten(), index=y_test.index)}
SVR_RBF_pred_test2_df = pd.DataFrame(data_SVR)
data_Ridge = {"(Ridge) predicted values": pd.Series(Ridge_pred_test10W.flatten(), index=y_test.index), "names": y_test.index.tolist()}
Ridge_pred_test2_df = pd.DataFrame(data_Ridge)
data_y = {"Actual values": y_test, "(Y - SVR-RBF) Residuals": y_test - pd.Series(SVR_RBF_pred_test10W.flatten(), index=y_test.index), "(Y - Ridge) Residuals": y_test - pd.Series(Ridge_pred_test10W.flatten(), index=y_test.index)}
data_y_df = pd.DataFrame(data_y)
prediction_10W = pd.concat([SVR_RBF_pred_test2_df, Ridge_pred_test2_df["(Ridge) predicted values"], data_y_df], axis=1)


# Metrics for SVR-RBF
SVR_RBF_mse_test2 = mean_squared_error(y_test, SVR_RBF_pred_test10W)
SVR_RBF_mse_train2 = mean_squared_error(y_train, SVR_RBF_pred_train10W)
SVR_RBF_r2_test2 = SVR_RBF_10W.score(X_test, y_test)
SVR_RBF_r2_train2 = SVR_RBF_10W.score(X_train, y_train)

# Metrics for Ridge Regression
Ridge_mse_test2 = mean_squared_error(y_test, Ridge_pred_test10W)
Ridge_mse_train2 = mean_squared_error(y_train, Ridge_pred_train10W)
Ridge_r2_test2 = Ridge_10W.score(X_test, y_test)
Ridge_r2_train2 = Ridge_10W.score(X_train, y_train)
...
metrics_10W = pd.DataFrame([results_list2_ridge, results_list2_SVR])
metrics_10W.index = ['Ridge', 'SVR-RBF']

diff_bh_10W = metrics_Bha - metrics_10W
diff_bh_10W.reset_index(inplace=True)
diff_bh_10W.rename(columns={'index': 'Model'}, inplace=True)
...
results_df2_10Closest = pd.DataFrame(results_list_2, index=range(len(results_list_2)))
tot_results_df2_10Closest = results_df2_10Closest.drop('parameter', axis=1).groupby('model').mean()
difference_Bh_10C = tot_results_df_2_Bhatia - results_df2_10Closest  # WHEN NEGATIVE THE NEW MODEL IS BETTER for R2, opposite for RMSE
difference_Bh_10C.reset_index(inplace=True)
...
ridge_results = results_df2_10Closest[results_df2_10Closest['model'] == 'Ridge Regression']
...
# Show the plot
plt.savefig('Ridge_alpha_10close.png')
plt.show()
...
plt.xticks(xticks)
plt.axis('tight')
plt.savefig('Ridge_Path_Diagram10close.png')
plt.show()
...
for alpha_value in ridge_alphas:
    ...
    plt.savefig(f"LearnCurve_Ridge_10close.png")
    plt.show()
...
# Get SVR-RBF results
svr_results = results_df2_10Closest[results_df2_10Closest['model'] == 'SVR-RBF']
