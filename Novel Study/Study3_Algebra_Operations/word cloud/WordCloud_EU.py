...
for word in tqdm(valid_risk_source_name_2[0:5]):
    modify_word = model[word] + model['EU']
...
most_similar_words_df = pd.DataFrame(most_similar_words, columns=['EU risk sources'])
...
for word in tqdm(valid_risk_source_name_2):
    modify_word = model[word] + model['Europe']
    modify_word_vectors_2.append(modify_word)
...
second_most_similar_words_df = pd.DataFrame(second_most_similar_words, columns=['EU risk sources'])

second_most_similar_words_df.insert(0, 'Risk sources', valid_risk_source_name_2)

valid_name_A = second_most_similar_words_df['EU risk sources']
y2 = y.copy()
...
plt.axis('off')
plt.title('Words Associated with European Risk perception')
plt.savefig('wordcloud_studyEU.png')
plt.show()
