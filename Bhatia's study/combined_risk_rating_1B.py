...
from psychometric_1B import psy_df_B
...
psy_df1 = psy_df_B.copy()
psy_df1 = psy_df1.drop('mean_ratings', axis=1)

data_300dimB = pd.concat([data_300dimB, psy_df1], axis=1)
...
results_df_B_combo = pd.DataFrame(results_list_B, index=range(len(results_list_B)))
max_min_resultsBcomb = results_df_B_combo.groupby('model').agg({'mean_score_R2': ['max', 'min'], 'mean_score_RMSE': ['max', 'min']})
max_min_resultsBcomb.to_excel('/Users/ClaudioProiettiMercuri_1/Downloads/MAXmin_df_Bcomb.xlsx')
# Save the results dataframe to a csv file
# results_df_B_combo.to_csv('results_df_1B_COMBINED.csv', index=False)
